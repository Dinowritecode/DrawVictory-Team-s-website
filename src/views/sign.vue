<template class="dark">
  ...
  <transition name="el-fade-in">
    <div id="mask" v-if="onVerify"></div>
  </transition>
  <transition name="el-fade-in">
    <div id="verification_frame" v-if="onVerify" @keydown.esc="onVerify=false">
      <h1>|人机验证</h1>
      <p id='verification_desc'>为什么要进行人机验证？我们只是想确认您是否是机器人，避免向服务器发送重复的请求</p>
      <input placeholder='请输入验证码' v-model.trim="verificationCode"
             @keydown.enter="register">
      <img class='code' :src='verificationImg' alt="验证码">
  ...
  <transition name="el-fade-in">
    <div class="login_frame frame" v-show="!isReg">
      <p>登录</p>
      <div class="input_box">
        <input required v-model.trim="user.username">
        <span>账户名称</span>
      </div>
      <div class="input_box">
  ...
  <transition name="el-fade-in">
    <div class="reg_frame frame" v-show="isReg">
      ...
      <div class="input_box">
        <input required v-model.trim="user.email" @blur="checkEmail">
        <span>邮箱地址</span>
      ...
      <div class="input_box">
        <input type="password" required v-model.trim="password" @blur="checkPwdIsSame" @keydown.enter="verify">
        <span>确认密码</span>
